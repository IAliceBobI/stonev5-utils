...
exports.getCurrentDateTimeComponents = getCurrentDateTimeComponents;
exports.formatDate = formatDate;
exports.formatDateYYYYMMDDHHmmss = formatDateYYYYMMDDHHmmss;
exports.getDayStr = getDayStr;
exports.getTimeStr = getTimeStr;
exports.getDateFromYYYYMMDDHHmmss = getDateFromYYYYMMDDHHmmss;
exports.convertMinutesToTimeFormat = convertMinutesToTimeFormat;
exports.setTimeouts = setTimeouts;
exports.setTimeoutOnce = setTimeoutOnce;
exports.sleep = sleep;
 ...
 * @returns
 */
function getDate(deltaSecs, date, offsetHours) {
    if (date == null) {
        date = new Date();
    }
    else {
 ...
 * @returns
 */
function getCurrentDateTimeComponents(deltaSecs, date, offsetHours) {
 ...
 * @returns 格式化后的日期字符串
 */
function formatDate(deltaSecs, date, offsetHours) {
 ...
 * @returns 格式化后的日期字符串
 */
function formatDateYYYYMMDDHHmmss(deltaSecs, date, offsetHours) {
    const { year, monthPad, dayPad, hoursPad, minutesPad, secondsPad } = getCurrentDateTimeComponents(deltaSecs, date, offsetHours);
    return year + monthPad + dayPad + hoursPad + minutesPad + secondsPad;
}
/**
 ...
 * @returns 格式化后的日期字符串
 */
function getDayStr(deltaSecs, date, offsetHours) {
 ...
 * @returns 格式化后的日期字符串
 */
function getTimeStr(deltaSecs, date, offsetHours) {
 ...
 * @returns 日期
 */
function getDateFromYYYYMMDDHHmmss(dateStr) {
...
function setTimeouts(cb, start, end, step) {
 ...
 */
function setTimeoutOnce(cb, start, end, step) {
    // 存储所有超时ID以便取消
    const timeoutIds = [];
...
function readableDuration(secs, partLen = 10) {
    if (secs <= 0)
        return '0s';
    const units = [
